...
public class SQLitePlugin {
    ...
    public String Callback(String status) {
    ...
    public void open(ReadableMap args, Callback success, Callback error) {
        ...
        try {
            this.execute(actionAsString, args, new Callback(success, error));
        } catch (Exception ex){
     ...
     */
    protected boolean execute(String actionAsString, ReadableMap args, Callback cbc) throws Exception{
    ...
    private boolean executeAndPossiblyThrow(Action action, ReadableMap args, Callback cbc){
        String dbname;
            ...
            case backgroundExecuteSqlBatch:
                ...
                if (r != null) {
                    try {
                        ...
                        Log.e(TAG, "couldn't add to queue", ex);
    ...
    private void createFromAssets(String dbName, File dbfile, InputStream assetFileInputStream) throws Exception {
        ...
        try {
            Log.v(TAG, "Copying pre-populated DB content");
            ...
            Log.v(TAG, "Copied pre-populated DB asset to: " + newDbFile.getAbsolutePath());
        } finally {
            closeQuietly(out);
        }
    }
     ...
     */
    private void closeDatabase(String dbName, Callback cbc) {
        DBRunner r = dbrmap.get(dbName);
        if (r != null) {
            try {
                ...
                Log.e(TAG, "couldn't close database", ex);
            }
        } else {
            if (cbc != null) {
     ...
     * @param cbc - JS callback
     */
    private void attachDatabase(String dbName, String dbNameToAttach, String dbAlias, Callback cbc) {
     ...
     * @param cbc - callback
     */
    private void deleteDatabase(String dbname, Callback cbc) {
        DBRunner r = dbrmap.get(dbname);
        if (r != null) {
            try {
            ...
            try {
                ...
                if (queryType == QueryType.update || queryType == QueryType.delete) {
                    ...
                    try {
                        ...
                        Log.e(TAG, "SQLiteStatement.executeUpdateDelete() failed", ex);
                        needRawQuery = false;
                    } finally {
                ...
                // INSERT:
                    ...
                    try {
                        ...
                        errorMessage = ex.getMessage();
                        Log.e(TAG, "SQLiteDatabase.executeInsert() failed", ex);
                    } finally {
                        closeQuietly(myStatement);
                    }
                }
                    ...
                    try {
                        ...
                        errorMessage = ex.getMessage();
                        Log.e(TAG, "SQLiteDatabase.beginTransaction() failed", ex);
                    }
                }
                    ...
                    try {
                        ...
                        errorMessage = ex.getMessage();
                        Log.e(TAG, "SQLiteDatabase.setTransactionSuccessful/endTransaction() failed", ex);
                    }
                }
                    ...
                    try {
                        ...
                        errorMessage = ex.getMessage();
                        Log.e(TAG, "SQLiteDatabase.endTransaction() failed", ex);
                    }
                }
                ...
                errorMessage = ex.getMessage();
                Log.e(TAG, "SQLitePlugin.executeSql[Batch](): failed", ex);
            }
    ...
    private class DBRunner implements Runnable {
        ...
        DBRunner(final String dbname, ReadableMap options) {
            this.dbname = dbname;
            ...
            try {
                ...
                Log.e(TAG,"Error retrieving assetFilename or mode from options:",ex);
            }
            ...
            if (this.androidLockWorkaround)
                Log.i(TAG, "Android db closing/locking workaround applied");
        ...
        public void run() {
            try {
                ...
                Log.e(TAG, "SQLite error opening database, stopping db thread", ex);
                ...
                Log.e(TAG, "Unexpected error opening database, stopping db thread", ex);
                if (openCbc != null) {
            ...
            try {
                ...
                while (!dbq.stop) {
                    ...
                    if (androidLockWorkaround && dbq.queries.length == 1 && dbq.queries[0].equals("COMMIT")) {
                        // Log.v(TAG, "close and reopen db");
                        closeDatabaseNow(dbname);
                        this.mydb = openDatabase(dbname, "", this.openFlags, null);
                        // Log.v(TAG, "close and reopen db finished");
                    }
                ...
                try {
                    ...
                    if (!dbq.delete) {
                        ...
                        try {
                            ...
                            Log.e(TAG, "couldn't delete database", ex);
    ...
    private final class DBQuery {
        ...
        DBQuery(String[] myqueries, String[] qids, ReadableArray[] params, Callback c) {
            this.stop = false;
            this.close = false;
            this.delete = false;
        ...
        DBQuery(boolean delete, Callback cbc) {
            this.stop = true;
            this.close = true;
            this.delete = delete;
            this.queries = null;
